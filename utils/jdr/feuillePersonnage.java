...
public class feuillePersonnage implements Serializable {
    ...
    public feuillePersonnage(String name,String author) {
        setName(name);
        this.player = author;
    }
    ...
    private int getLifePoint() {
        return lifePoints;
    }

    private int getManaPoints() {
        return manaPoints;
    }

    private List<String> getStatNames() {
        return new ArrayList<String>(this.stats.keySet());
    }

    private int getStatValue(String carac) {
        return this.stats.get(carac);
    }
    ...
    private int getCompValue(String comp) {
        return this.comp.get(comp);
    }

    private List<String> getInvNames() {
        return new ArrayList<String>(inv.keySet());
    }

    private int getInvNb(String obj) {
        return this.inv.get(obj);
    }
    ...
    public void setStats(String carac, Integer valeur) {
        if (this.stats.containsKey(carac)) {
            this.stats.replace(carac, valeur);
        } else {
            this.stats.put(carac, valeur);
        }
    }

    public void setComp(String comp, Integer valeur) {
        if (this.comp.containsKey(comp)) {
            this.comp.replace(comp, valeur);
        } else {
            this.comp.put(comp, valeur);
        }
    }

    public void setInv(String obj, Integer valeur) {
        if (this.inv.containsKey(obj)) {
            this.inv.replace(obj, valeur);
        } else {
            this.inv.put(obj, valeur);
        }
    }
